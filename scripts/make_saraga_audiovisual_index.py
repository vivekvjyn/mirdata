...
video_checksums = pd.read_csv("checksums.csv", names=["path", "checksum"], header=None)


def make_saraga_audiovisual_index(dataset_path):
    saraga_index = {
        "version": 1.0,
        "tracks": {},
    }
    idx = 0
    dataset_path_prev = dataset_path.split(DATASET)[0]

    for concert in os.listdir(dataset_path + " audio"):
        for song in os.listdir(os.path.join(dataset_path + " audio", concert)):
            ...
            for file in os.listdir(
                os.path.join(dataset_path + " audio", concert, song)
            ):
                # Audio
                if ".wav" in file:
                    if file == song + ".wav":
                        audio_path = os.path.join(
                            DATASET + " audio", concert, song, file
                        )
                        audio_checksum = md5(
                            os.path.join(dataset_path_prev, audio_path)
                        )
                    ...
                    if "mridangam-left" in file:
                        ...
                        audio_mridangam_left_checksum = md5(
                            os.path.join(dataset_path_prev, audio_mridangam_left_path)
                        )
                    ...
                    if "mridangam-right" in file:
                        ...
                        mridangam_right_checksum = md5(
                            os.path.join(dataset_path_prev, mridangam_right_path)
                        )
                    ...
                    if "violin" in file:
                        ...
                        audio_violin_checksum = md5(
                            os.path.join(dataset_path_prev, audio_violin_path)
                        )
                    ...
                    if "vocal" in file:
                        ...
                        audio_vocal_checksum = md5(
                            os.path.join(dataset_path_prev, audio_vocal_path)
                        )
            ...
            for file in os.listdir(
                ...
                # Gesture
                if file == "mridangam":
                    for gesture_file in os.listdir(
                        os.path.join(dataset_path + " gesture", concert, song, file)
                    ):
                        if "kpts.npy" in gesture_file:
                            keypoints_mridangam_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            keypoints_mridangam_checksum = md5(
                                os.path.join(
                                    dataset_path_prev, keypoints_mridangam_path
                                )
                            )
                            keypoints_mridangam = (
                                keypoints_mridangam_path,
                                keypoints_mridangam_checksum,
                            )
                        if "scores.npy" in gesture_file:
                            scores_mridangam_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            scores_mridangam_checksum = md5(
                                os.path.join(dataset_path_prev, scores_mridangam_path)
                            )
                            scores_mridangam = (
                                scores_mridangam_path,
                                scores_mridangam_checksum,
                            )
                if file == "singer":
                    for gesture_file in os.listdir(
                        os.path.join(dataset_path + " gesture", concert, song, file)
                        ...
                        if "kpts.npy" in gesture_file:
                            keypoints_singer_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            keypoints_singer_checksum = md5(
                                os.path.join(dataset_path_prev, keypoints_singer_path)
                            )
                            keypoints_singer = (
                                keypoints_singer_path,
                                keypoints_singer_checksum,
                            )
                        if "scores.npy" in gesture_file:
                            scores_singer_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            scores_singer_checksum = md5(
                                os.path.join(dataset_path_prev, scores_singer_path)
                            )
                ...
                if file == "violin":
                    for gesture_file in os.listdir(
                        ...
                        if "kpts.npy" in gesture_file:
                            keypoints_violin_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            keypoints_violin_checksum = md5(
                                os.path.join(dataset_path_prev, keypoints_violin_path)
                            )
                            keypoints_violin = (
                                keypoints_violin_path,
                                keypoints_violin_checksum,
                            )
                        if "scores.npy" in gesture_file:
                            scores_violin_path = os.path.join(
                                DATASET + " gesture", concert, song, file, gesture_file
                            )
                            scores_violin_checksum = md5(
                                os.path.join(dataset_path_prev, scores_violin_path)
                            )
                            scores_violin = (scores_violin_path, scores_violin_checksum)

            for file in os.listdir(
                os.path.join(dataset_path + " metadata", concert, song)
            ):
                # Metadata
                if ".json" in file and not file.startswith("._"):
                    metadata_path = os.path.join(
                        DATASET + " metadata", concert, song, file
                    )
                    metadata_checksum = md5(
                        os.path.join(dataset_path_prev, metadata_path)
                    )
